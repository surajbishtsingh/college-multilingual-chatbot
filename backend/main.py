...
# ── Local imports (all relative to backend/) ──────────────────────────
from language_detector import detect_language
from rag.kb_query import get_answer, get_qdrant, get_embed_model
from voice import generate_voice
from memory.database import init_db
from memory.memory_manager import (
    process_user_message, process_bot_message, build_memory_context
)
from scraper.scheduler import (
    start_scheduler, stop_scheduler, get_scrape_status, run_scrape_job
)
...
async def chat(request: ChatRequest, req: Request):
    ...
    # ✅ These 3 lines must exist for memory to work
    await process_user_message(session_id, question, lang)      # save + extract facts
    memory_context = await build_memory_context(session_id)     # load memory
    answer = await run_in_threadpool(get_answer, question, lang, memory_context)
    await process_bot_message(session_id, answer, lang)         # save response
